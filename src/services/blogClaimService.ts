...
export class BlogClaimService {
  ...
  static async getUserClaimStats(userId: string): Promise<UserClaimStats> {
    try {
      // Count user's claimed posts from database
      ...
      console.log('📁 Using localStorage fallback for claim stats');
  ...
  static async claimBlogPost(blogSlug: string, userId: string): Promise<ClaimResult> {
    try {
      ...
      // Try to claim from database first
      try {
        ...
        console.log('✅ Blog post claimed successfully in database');
        return {
          success: true,
        ...
        console.log('✅ Blog post claimed successfully in localStorage');
        return {
          success: true,
          message: 'Blog post claimed successfully! It is now permanently yours.'
        };
  ...
  /**
   * Unclaim a blog post (only if claimed by the user)
   */
  static async unclaimBlogPost(blogSlug: string, userId: string): Promise<ClaimResult> {
    try {
      // Try database first
      try {
        const { data: existingPost, error: fetchError } = await supabase
        ...
        if (fetchError || !existingPost) {
          throw new Error('Blog post not found or not owned by user');
        }

        // Unclaim the post (set expiration back to 24 hours)
        ...
        return {
          success: true,
          message: 'Blog post unclaimed successfully. It will expire in 24 hours if not claimed again.'
        };
        ...
        if (blogPost.user_id !== userId) {
          return {
            success: false,
            message: 'You can only unclaim posts that you own.'
          };
        }

        // Unclaim the post
        blogPost.user_id = null;
        blogPost.expires_at = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
        blogPost.updated_at = new Date().toISOString();
      ...
      console.error('❌ Failed to unclaim blog post:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error occurred'
      };
