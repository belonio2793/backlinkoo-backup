...
Focus on ranking factors while maintaining user value and engagement.`;
  ...
  private getSystemPrompt(provider: string, request: ContentRequest): string {
    ...
    return `${basePrompt} Focus on expertise, authoritativeness, and trustworthiness. Use data-driven insights, professional tone, and comprehensive coverage. Create content that demonstrates deep subject matter expertise.`;
  }
  ...
  async generateContent(request: ContentRequest): Promise<AIContentResult> {
    ...
    const generationPromises = this.providers.map(async (provider, index) => {
      ...
      try {
        ...
        // Only OpenAI provider
        const result = await provider.service.generateContent(selectedPrompt, {
